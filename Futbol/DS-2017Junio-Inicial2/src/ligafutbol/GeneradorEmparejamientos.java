...
public class GeneradorEmparejamientos {
		...
		for (Equipo equipo : equipos) {
			for (Equipo candidato : equipos) {
				if (equipo != candidato && !emparejamientos.get(equipo).contains(candidato)) {
					...
					if (validadores(equipo, candidato)) {
	...
	private boolean validadores(Equipo equipo, Equipo candidato) {
		Validador valid = new ValidadorAll(new ValidadorEdad(), new ValidadorMasculino());
		return valid.validar(equipo, candidato);
	}
}
